...
class Pred_endecoder(nn.Module):
    ...
    def __init__(self, channel, latent_dim):
        ...
        self.noise_conv = nn.Conv2d(3 + latent_dim, 3, kernel_size=3, padding=1)
    ...
    def forward(self, x, z):
        ...
        z_noise = self.tile(z_noise, 2, x.shape[self.spatial_axes[0]])
        z_noise = torch.unsqueeze(z_noise, 3)
        z_noise = self.tile(z_noise, 3, x.shape[self.spatial_axes[1]])

        x = torch.cat((x, z_noise), 1)
        x = self.noise_conv(x)
        ...
        # z_noise = torch.unsqueeze(z, 2)
        # z_noise = self.tile(z_noise, 2, x1.shape[self.spatial_axes[0]])
        # z_noise = torch.unsqueeze(z_noise, 3)
        # z_noise = self.tile(z_noise, 3, x1.shape[self.spatial_axes[1]])
        #
        # conv1_feat = torch.cat((conv1_feat, z_noise), 1)
        # conv1_feat = self.noise_conv1(conv1_feat)

        # z_noise = torch.unsqueeze(z, 2)
        # z_noise = self.tile(z_noise, 2, x2.shape[self.spatial_axes[0]])
        # z_noise = torch.unsqueeze(z_noise, 3)
        # z_noise = self.tile(z_noise, 3, x2.shape[self.spatial_axes[1]])
        #
        # conv2_feat = torch.cat((conv2_feat, z_noise), 1)
        # conv2_feat = self.noise_conv2(conv2_feat)
        #
        # z_noise = torch.unsqueeze(z, 2)
        # z_noise = self.tile(z_noise, 2, x3.shape[self.spatial_axes[0]])
        # z_noise = torch.unsqueeze(z_noise, 3)
        # z_noise = self.tile(z_noise, 3, x3.shape[self.spatial_axes[1]])
        #
