...
for epoch in range(1, (opt.epoch+1)):
    ...
    loss_lat_record = AvgMeter()
    loss_reg_record = AvgMeter()
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            pred_prior, pred_post, latent_loss = generator(images,gts)
            reg_loss = l2_regularisation(generator.enc_x) + \
                       l2_regularisation(generator.enc_xy) + l2_regularisation(generator.prior_dec) + l2_regularisation(generator.post_dec)
            ...
            if rate == 1:
                ...
                loss_lat_record.update(loss_latent.data, opt.batchsize)
                loss_reg_record.update(reg_loss.data, opt.batchsize)

        if i % 10 == 0 or i == total_step:
            print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Gen Loss: {:.4f}, lat Loss: {:.4f}, Reg Loss: {:.4f}'.
                  format(datetime.now(), epoch, opt.epoch, i, total_step, loss_record.show(),loss_lat_record.show(), loss_reg_record.show()))
