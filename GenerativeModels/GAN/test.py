...
discriminator.load_state_dict(torch.load('./models/Model_50_dis.pth'))
...
for dataset in test_datasets:
    save_path = './results_gen/' + dataset + '/'
    #save_path = './results/ResNet50/holo/train/left/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    save_path_dis = './results_dis/' + dataset + '/'
    # save_path = './results/ResNet50/holo/train/left/'
    ...
    for i in range(test_loader.size):
        ...
        dis_pred = torch.sigmoid(generator_pred)
        Dis_output = discriminator(image, dis_pred)
        res = generator_pred
        res = F.upsample(res, size=[WW, HH], mode='bilinear', align_corners=False)
        res = torch.abs(res.sigmoid()-0.5).data.cpu().numpy().squeeze()
        res = 255*(res - res.min()) / (res.max() - res.min() + 1e-8)
        cv2.imwrite(save_path_dis + name, res)