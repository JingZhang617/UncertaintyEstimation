...
class Pred_endecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        mask1 = Masksembles2D(self.channel,conv1_feat.shape[0],2.0).cuda()
        conv1_feat = mask1(conv1_feat)
        conv2_feat = self.conv2(x2)
        mask2 = Masksembles2D(self.channel, conv2_feat.shape[0], 2.0).cuda()
        conv2_feat = mask2(conv2_feat)
        conv3_feat = self.conv3(x3)
        mask3 = Masksembles2D(self.channel, conv3_feat.shape[0], 2.0).cuda()
        conv3_feat = mask3(conv3_feat)
        conv4_feat = self.conv4(x4)
        mask4 = Masksembles2D(self.channel, conv4_feat.shape[0], 2.0).cuda()
        conv4_feat = mask4(conv4_feat)
